#!/usr/bin/env python3
"""
Readaptation of the cologne-phonetics with a more strict coding for calculation of fuzzy similarity.

Cologne phonetics considers a wider range of sounds similar, due to the fact that it was
designed for creating phonetic keys for indexing and exact match search.

A detailed explanation of the cologne phonetics can be found at:
https://en.wikipedia.org/wiki/Cologne_phonetics

However, when calculating a "phonetic similarity" score, this would be less desirable.
Therefore this module implements a similar algorithm which drops certain rules, with the goal of
creating a more strict phoenetic key.

The implmentation is derived from https://github.com/provinzkraut/cologne_phonetics
"""
...
RGX_RULES = [
    ...
    # vowels codes should be preserved and distinguished between each other
    # old rule:
