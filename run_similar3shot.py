"""
Reproduce Llama-3-8B-prompt2-similar3shot from compare.ipynb using SimilarExamples
(embedding-based) with num_examples=3.

Predictions are saved to preds_similar3shot_val.json.
Metrics are saved to metrics_similar3shot_val.txt.
"""
import json
import time
import datetime
...
NUM_EXAMPLES        = 3
BATCH_SIZE          = 32
PREDS_OUTPUT_PATH   = Path("preds_similar3shot_v2_val.json")
METRICS_OUTPUT_PATH = Path("metrics_similar3shot_v2_val.txt")
...
n_addrs = {"train": len(bzkopen_train), "val": len(bzkopen_val)}
addresses_per_card_total = sum(n_addrs[s] / n_cards[s] for s in n_addrs)
...
if PREDS_OUTPUT_PATH.exists():
    ...
    print("Building SimilarExamples strategy...")
    example_strategy = SimilarExamples(
