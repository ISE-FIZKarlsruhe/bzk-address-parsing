...
Run address component extraction using HybridSimilarExamples:
  - 2 shots selected by SimilarExamples (sentence-embedding similarity)
  - +1 bonus shot from PatternTokenSimilarExamples (regex pattern), only when
    pattern_score == 1.0 (exact structural match) and not already in the 2 above

Predictions are saved to preds_hybrid3shot_val.json. Evaluation metrics
...
CONFIG_NAME         = "Llama-3-8B-prompt2-hybrid3shot"
MODEL_NAME          = "meta-llama/Meta-Llama-3-8B-Instruct"
N_EMBEDDING         = 2
N_PATTERN_BONUS     = 1
BONUS_PATTERN_THRESHOLD = 1.0
BATCH_SIZE          = 32
PREDS_OUTPUT_PATH   = Path("preds_hybrid3shot_v2_val.json")
METRICS_OUTPUT_PATH = Path("metrics_hybrid3shot_v2_val.txt")
...
if PREDS_OUTPUT_PATH.exists():
    ...
    print("Building PatternTokenSimilarExamples (bonus) strategy...")
    pattern_strategy = PatternTokenSimilarExamples(
        example_addresses=bzkopen_train["FullAddress"],
        example_labels=bzkopen_train,
        num_examples=N_PATTERN_BONUS,
        labels_to_include=ENTITIES_TO_PREDICT,
    )
